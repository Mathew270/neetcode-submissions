...
class Solution:
    def findWords(self, board: List[List[str]], words: List[str]) -> List[str]:
        ...
        def dfs(x,y, word, node):
            ...
            cur = node.children[board[x][y]]    # since board[x][y] is a child set curr to that
            word += board[x][y]                 # update formed word
            if cur.isEnd:                       # if current letter (node) is last then add to res
                res.add(word)

            # even if letter was last continue search since that
            # could be prefix of another word still yet to be added
